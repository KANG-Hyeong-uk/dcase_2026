baseline 수정사항
원인: 원본 train_test.py의 train_model() 함수 안에서 hidden_size, dropout, mode, class_dict 등 변수를 참조하는데, 이 변수들은 __main__ 블록에서만 정의되고 함수 파라미터로 전달되지 않아서 NameError 발생. 노트북 패치 셀이 추가로 hidden_size=HIDDEN 같은 코드를 잘못 합쳐서 더 망가진 상태였습니다.

수정 내용: model_config를 구성할 때 train_model이 파라미터로 받은 model 객체의 속성(model.hidden_size, model.num_classes 등)에서 값을 읽도록 변경했습니다. BaseClassifier는 __init__에서 이 속성들을 모두 self.*로 저장하고 있으므로 정상 동작합니다.

이제 셀 11을 다시 실행하면 됩니다
